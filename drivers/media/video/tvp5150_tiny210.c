...
static int tvp5150_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
...
#if	0
			printk("%d: %d ---- (%d, %d)\n", i, ctrl->value,
					tvp5150_qctrl[i].minimum, tvp5150_qctrl[i].maximum);
#endif
			return -ERANGE;
		}
	...
	/* DON'T return -EINVAL for android */
	v4l2_dbg(1, debug, sd, "Unsupported CTRL (%08x)\n", ctrl->id);
	return 0;
}

static int tvp5150_init(struct v4l2_subdev *sd, u32 val)
{
	v4l2_info(sd, "camera initialization start (%08x)\n", val);
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < TVP5150_REG_NUM; i++) {
		val = tvp5150_read(sd, i);
...
static ssize_t tvp5150_reg_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	...
	if (reg < 0 || reg >= TVP5150_REG_NUM) {
		return -EINVAL;
	}

	tvp5150_write(sd, reg, val);
	new = tvp5150_read(sd, reg);
	printk("%02x  %02x (%02x)\n", reg, new, val);

	return strnlen(buf, count);
}

static DEVICE_ATTR(tvp5150_reg, S_IRUGO | S_IWUSR, tvp5150_reg_show, tvp5150_reg_store);
			 ...
			 const struct i2c_device_id *id)
{
	...
	/* Power on */
	gpio_request(S5PV210_GPJ3(1), "GPJ3_1");
	gpio_direction_output(S5PV210_GPJ3(1), 0);
	//printk("CAM_A_RESET: low\n");
	udelay(50);
	gpio_direction_output(S5PV210_GPJ3(1), 1);
	//printk("CAM_A_RESET: high\n");
...
#define __V(sd, reg)	reg, tvp5150_read(sd, reg)
		printk("R 0x%02x = %02x\n", __V(sd, 0x80));
		printk("R 0x%02x = %02x\n", __V(sd, 0x81));
	...
	if (device_create_file(&c->dev, &dev_attr_tvp5150_reg)) {
		dev_err(&c->dev, "Create sysfs file failed\n");
	}
