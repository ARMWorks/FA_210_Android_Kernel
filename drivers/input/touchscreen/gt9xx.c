...
{
	s32 ret = -1;
	struct gt9xx_i2c_platform_data *pdata;
	struct goodix_ts_data *ts;
	unsigned int ctp_id;
	u16 version_info;
	...
	if (ctp_id != CTP_GT9XX && ctp_id != CTP_AUTO) {
		return -ENODEV;
	}
	...
	if (pdata) {
		...
		if (pdata->irq_cfg) {
			s3c_gpio_cfgpin(pdata->gpio_irq, pdata->irq_cfg);
			s3c_gpio_setpull(pdata->gpio_irq, S3C_GPIO_PULL_NONE);
		}
#endif
	} else {
